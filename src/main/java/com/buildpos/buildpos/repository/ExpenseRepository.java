...
public interface ExpenseRepository extends JpaRepository<Expense, Long> {
    ...
    @Query("SELECT COALESCE(SUM(e.amount), 0) FROM Expense e WHERE e.shift.id = :shiftId AND e.paymentMethod = :method")
    BigDecimal sumByShiftAndMethod(@Param("shiftId") Long shiftId, @Param("method") com.buildpos.buildpos.entity.enums.PaymentMethod method);
