...
import {
    Plus, Minus, ImageIcon, Wand2, Shuffle, ScanLine,
    Loader2, Save
} from 'lucide-react'
...
const ProductForm = forwardRef(function ProductForm({
    ...
    const buildInitialForm = () => {
        const base = { ...EMPTY_FORM, units: [{ ...EMPTY_UNIT, isBaseUnit: true }] }
        if (!initialValues) return base
        return {
            ...base,
            ...initialValues,
            units: initialValues.units
                ? initialValues.units.map((u, i) => ({ ...EMPTY_UNIT, ...u, isBaseUnit: u.isBaseUnit ?? (i === 0) }))
                : base.units,
        }
    }
    ...
    }, [form, exchangeRate, stockAdjustments, isEdit, productId, onSaved, t])

    useImperativeHandle(ref, () => ({
        save: handleSave,
        isSaving: () => saving,
        isUploading: () => imageUploading,
    }), [handleSave, saving, imageUploading])
